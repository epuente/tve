...
$(document).on("click","#btnPruebaCorreo", function(event){
	...
	|| $("#txtPara").val()=="" ){
		...
		$("#msgPruebaCorreo").text("Preparando el envío...").css({'color': ''});
		var $envio=LlamadaAjax("/envioCorreoPrueba","POST", JSON.stringify(x));
		$.when($envio).done(function(data){
			console.dir(data);
			if (data.enviado){
				$("#msgPruebaCorreo").html("<div>Se envió con éxito el correo.</div><div>Lo cual indica que la configuración de hostname, puerto, cuenta y clave son los correctos.</div>").css("color", "green");
			} else {
				$("#msgPruebaCorreo").text("Ocurrió un error, no se pudo enviar el correo.").css("color", "red");
			}
		});
	}
