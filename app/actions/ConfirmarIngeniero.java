...
public class ConfirmarIngeniero extends Action.Simple {  
    public F.Promise<Result> call(play.mvc.Http.Context ctx) throws Throwable {
        ...
        if (play.mvc.Controller.session("rolActual").compareTo("18")==0) {
        	System.out.println("desde ConfirmarIngeniero "+ctx.request().path()+"  "+new Date());
     ...
     //   Result unauthorized = Results.unauthorized("RegistroAcceso no autorizado, requiere iniciar sesión como usuario autorizado.");
        System.out.println("Se intentó ejecutar un método que solo es accesible al rol de ingeniero.");
